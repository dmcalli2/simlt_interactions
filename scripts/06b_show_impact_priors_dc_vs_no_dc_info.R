...
scenarios <- c('atc5_0.05_trial_0.05_drug_0.05','atc5_0.25_trial_0.25_drug_0.25') # Using only a single scenario for now (median variation) because
                                                 # 'truth' at the DC/drug level will change across scenarios at random
                                                 # (even when selected as closest to mean WDG effect)
...
# 3 A10BX NCT00568984 24repaglinide  NA
 ...
 # dc2 <- inla.make.lincomb(myatc4 = 1, myatc5 = c(NA, NA, NA, NA, NA, NA, 1)) # DC level   #Not applicable
  
  dc3 <- inla.make.lincomb(myatc4 = 1,   # DC new drug new class
                           mydrug = c(rep(NA, 21), 1,NA,NA, NA)) 
  
  dc4 <- inla.make.lincomb(myatc4 = 1,  # DC new drug ext class
                           mydrug = c(rep(NA, 24), 1)) 
 ...
 # dc2 <- inla.make.lincomb(myatc4 = 1, myatc5 = 1) # DC level    #  Not applicable
  
 # dc3 <- inla.make.lincomb(myatc4 = 1, myatc5 = c(NA, NA, NA, NA, NA, NA, 1),  # Not applicable
 #                          mydrug = c(rep(NA, 21), 1,NA,NA, NA)) 
  
  dc4 <- inla.make.lincomb(myatc4 = 1,  # DC new drug ext class - should be same as dc2
                           mydrug = c(NA,NA,NA, 1)) 
  ## Run model, trial within drug within a single ATC5 class (for new drug in existing DC)
  mod3_nested <- inla(myform_nested3, 
                      data = my_data2,
                      # Add linear combinations to estimate drug-class
                      lincomb = c(a = dc1,  d=dc4),
                      # Likelihood distribution
                      family = "gaussian",
                      # Fix likelihood hyperpars as the data is fixed with known precision.
                      control.family = list(hyper = list(prec = list(fixed = TRUE, initial = 0))),
  ...
  list(a10bx = a10bx, mdl1 = mod1_nested, mdl2 = mod2_nested ,mdl3 = mod3_nested, scenario= scenario, iter=iter)
})
mdls <- transpose(mdls)
a10bx <- mdls$a10bx
scns <- mdls$scenario
