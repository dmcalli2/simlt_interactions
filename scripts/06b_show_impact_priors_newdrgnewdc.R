...
# # 3 A10BX NCT00568984 24repaglinide  NA
  ...
  # Add in 3 trials of a new drug in new class
  
  new_d <- as.data.frame(matrix(nrow=3,data=c(mean(my_data$y_prec),162,1,8,25,NA,
                                              mean(my_data$y_prec),163,1,8,25,NA,
                                              mean(my_data$y_prec),164,1,8,25,NA), byrow=TRUE))
  ...
  dc2 <- inla.make.lincomb(myatc4 = 1, myatc5 = c(NA, NA, NA, NA, NA, NA, NA, 1)) # DC level, new drug in a new class
  
  dc3 <- inla.make.lincomb(myatc4 = 1, myatc5 = c(NA, NA, NA, NA, NA, NA, NA, 1),  # drug level,  new drug new class  
  ...
  dc2 <- inla.make.lincomb(myatc4 = 1) # DC level   #Not applicable
  ...
  list( mdl1 = mod1_nested, mdl2 = mod2_nested , scenario= scenario, iter=iter)
})
mdls <- transpose(mdls)
...
mdl_t_tdy <- mdl_t_tdy %>%
  mutate( b.lc.x = ifelse(model == "Full, no DC info",NA,b.lc.x),
          b.lc.y = ifelse(model == "Full, no DC info",NA,b.lc.y))
...
ggplot(mdl_t_tdy) +
  ...
  geom_line( aes(x=b.lc.x,y=b.lc.y, colour = "NewDrug_NewDC_DClevel")) +
  geom_line( aes(x=c.lc.x,y=c.lc.y, colour= "NewDrug_NewDC_Druglevel")) +
  scale_colour_manual("", 
                      breaks = c("WDG", "NewDrug_NewDC_DClevel", "NewDrug_NewDC_Druglevel"),
...
mdl_t <- map(mdls, function(mdl_each){
  ...
  # Parameters and data
  list(param = c(a,dc_a,ndnc_a), res = res_new)
})


names(mdl_t) <- paste(rep(c("Lo","Me","Hi"),  2), c( rep("Full", 3), rep("Full_noDCinfo",3)), sep = "_")
...
saveRDS(mdl_t, file = "scratch_data/Priors_for_newdrug_newclass.Rds")
