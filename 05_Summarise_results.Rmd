...
Using the following very simple simulation (the same fixed effect for all trials). For continuous outcomes I was able to recover wider-drug-grouping level estimates for models where trials was nested within wider drug grouping, condition and drug class, and where the interaction for each trial was treated as fixed. For categorical

```{r, fig.cap = "Trials"}
readRDS("scratch_data/simulated_data.Rds") %>% 
...
The model shows a really large standard error for the null interaction. I think I shoudl use a mildly informative prior. 