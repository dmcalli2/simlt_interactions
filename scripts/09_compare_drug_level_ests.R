...
mdls <- map(scenarios_random_iterations, function (i) {
  ...
  dc2 <- inla.make.lincomb(myatc4 = 1, myatc5 = c(NA, NA, NA, 1, NA, NA, NA), mydrug = c(rep(NA,20),1,NA,NA,NA) ) # drug level
...
for( i in 1:length(itrs)){
  fixeds <- rbind(fixeds, data.frame(summary(mdl2[[i]])$fixed))
