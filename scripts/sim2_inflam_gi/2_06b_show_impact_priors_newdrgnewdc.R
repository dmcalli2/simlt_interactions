...
#mdls <- map(scenarios_random_iterations, function (i) {
  ...
  new_d <- as.data.frame(matrix(nrow=3,data=c(mean(my_data$y_prec),163,1,18,33,2,NA,
                                              mean(my_data$y_prec),164,1,18,33,2,NA,
                                              mean(my_data$y_prec),165,1,18,33,2,NA), byrow=TRUE))
  ...
  dc3 <- inla.make.lincomb(wdg = 1, mypath = c(NA,1), mymoa = c(rep(NA,17),1 )) # DC level, new drug in a new class
  
  dc4 <- inla.make.lincomb(wdg = 1, mypath = c(NA,1), mymoa = c(rep(NA,17),1 ),  # drug level,  new drug new class  
...
mdl_t_tdy$model <- factor(mdl_t_tdy$model,levels(mdl_t_tdy$model)[c(2,1)]) 
...
names(mdl_t) <- paste(rep(c("Lo","Me","Hi"),  2), c( rep("Full", 3), rep("Full_noDC_Path_info",3)), sep = "_")
...
saveRDS(mdl_t, file = "scratch_data/Priors_for_newdrug_newclass_sim2.Rds")
